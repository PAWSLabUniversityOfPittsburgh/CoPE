 ...
 * @author Michael V. Yudelson
 */
public abstract class Node extends Item implements iNode
{
	
	private static final String NODE_RATING_LABELS = 
	...
	protected boolean folder_flag;

	/** Node properties
	 */
	protected NodeComment comments;
	protected NodeConceptsOld concepts;

	protected iNode parent;
	protected OrderedWeightedItemVector<iNode> children;

	protected boolean expanded = false; // property for tree visualization

	protected ItemVector<iConcept> ontologies; // ontologies of concepts
//	protected ItemVector<iConcept> concepts; // descriptive concepts

	protected iResource xtrnl_object;

	/** Rating of a group of users
	 * @since 1.5
	 */
	protected Rating group_rating;

	/** personal rating of a user
	 * @since 1.5
	 */
	protected Rating personal_rating;
	...
	public Node(int _id, String _title, int _node_type, String _descr,
		String _url, boolean _folder_flag, iResource _xtrnl_object)
	{
		super(_id, _title);
//		id = _id;
//		title = (_title==null)?"":_title;
		expanded = false;
		parent = null;
		children = new OrderedWeightedItemVector<iNode>();
		node_type = _node_type;
		description = (_descr==null)?"":_descr;
		url = (_url==null)?"":_url;
//System.out.println("Node.Node url " + url);
		folder_flag = _folder_flag;
		comments = new NodeComment(this);
		concepts = new NodeConceptsOld(this);
		ontologies = new ItemVector<iConcept>();
		setExternalObject(_xtrnl_object);

		group_rating = null;
		personal_rating = null;
	}

	public Node(int _id, String _title, iNode _parent, int _node_type, String _descr,
		String _url, boolean _folder_flag, iResource _xtrnl_object)
	{
		super(_id, _title);
//		id = _id;
...
//System.out.println("Node.Node url " + url);
	...
	public abstract iNode clone(User _user, boolean _set_xtrnal_obj);
...
//	public void setTitle(String _title) { title = _title; }
	...
	// HierarchicalItem casting
//	public void setParent(iNode _parent) { parent = _parent; }
//	public iNode getParent() { return parent; }
//	public ItemVector<iNode> getChildren() { return children; }


	public NodeComment getComment() { return comments ; }
	public void setComment(NodeComment _comments) { comments = _comments; }

	public NodeConceptsOld getConcepts() { return concepts ; }
	public void setConcepts(NodeConceptsOld _concepts) { concepts = _concepts; }

	public int compareTo(Object e) { return getTitle().compareTo(((Node)e).getTitle()); }
	...
	public iResource getExternalObject() { return xtrnl_object; }
	public void setExternalObject(iResource _xtrnl_object)
	{
		xtrnl_object = _xtrnl_object;
		if(_xtrnl_object!=null) xtrnl_object.getOwners().addNew(this);
//if(_xtrnl_object!=null && _xtrnl_object.getId()==22)System.out.println("Paper 22, Node " + this.getId() +  " owners#=" + _xtrnl_object.getOwners().size());
	}

	/**
	 * Method performs non-duplicating recursive addition of the node to a
	 * separate resource map. This method is used in creating a user copy
	 * of the resource map from a server copy.
	 * Method preserves child-parent relationships of only nodes are added
	 * in parent-first-child-next order
	 * @param resmap - resource map to copy node(s) to
	 * @param new_user - user who's 'veiw' of the resources should be copied
	 */
	...
	{
		// create and add a clone of the node
		iNode new_node = this.clone(new_user, false /*set external object*/);
		// add my profile if root
		if(this.getNodeType() == iNode.NODE_TYPE_I_ALL)
		{
//System.out.println("Node.copyToResourceMap clonind root node");			
			// Add MyProfile node
			iNode myprofile = new MyProfileNode(-resmap.getNextVirtualNodeId(), new_user);
			resmap.getNodes().add(myprofile);
			new_node.getChildren().add(0, myprofile);
			myprofile.setParent(new_node);
			
//			// add "My Documents"
//			iNode my_documents = new NodeUntyped(
//					-resmap.getNextVirtualNodeId() /*_id*/,
//					"My Documents" /*_title*/,
//					iNode.NODE_TYPE_I_FOLDER /*_node_type*/,
//					"" /*_descr*/,
//					"" /*_url*/,
//					true /*_folder_flag*/,
//					"" /*_properties*/,
//					null /*_user*/,
//					null /*_xtrnl_object*/);
//			resmap.getNodes().add(my_documents);
//			new_node.getChildren().add(1,my_documents);
//			my_documents.setParent(new_node);
		}
		
		// copy node's ontologies
		for(int i=0; i<this.ontologies.size(); i++)
		{
			ItemVector<iConcept> onto_concepts =
				this.ontologies.get(i).copyConceptTree(null, null);
			new_node.getOntologies().addNew( onto_concepts.get(0) );
			copy_concepts.addAllNew(onto_concepts);
		}
//System.out.println("Node.copyToResourceMap node#" +  this.getId() + " ontologies#=" + this.ontologies.size() + " copied concepts#=" + copy_concepts.size());

		if(new_user != null)
		{
			if(this.creator.getId() == new_user.getId())
				new_node.setUser(new_user);
			else if (can_see_authors)
			{
				int owner_id = this.getCreator().getId();
				User new_owner = resmap.getUsers().findById(owner_id);
				new_node.setUser(new_owner);
			}
		}
//System.out.println("Node.copyToResourceMap (new_node==null)" + (new_node==null));
//if(new_node!=null)System.out.println("Node.copyToResourceMap (new_node.getId())" + (new_node.getId()));
		resmap.getNodes().addNew(new_node);

		// preserve parent-node relationships
//		if(resmap.getNodes().findById(this.parent.getId()) != null)
//			new_node.setParent(
//				resmap.getNodes().findById(this.parent.getId()) );

		// recurse on children
//System.out.println("Node.copyToResourceMap children.size() " + children.size());
		for(int i=0; i<children.size() ; i++)
		{
//System.out.println("children.get(i)=null:" + (children.get(i)==null) + " parent id=" + this.getId() + " children#" + children.size() + " i="+i);
			if(children.get(i)==null)
				continue;
			copy_concepts = children.get(i).copyToResourceMap(resmap,
					new_user, copy_concepts, can_see_authors);
		}

		return copy_concepts;
	}
	...
	{
		if(_ontologies==null)
			_ontologies = new ItemVector<iConcept>();
		for(int i=0; i<this.ontologies.size(); i++)
			_ontologies.add(this.ontologies.get(i));

		if(this.getParent() != null)
			_ontologies = ((iNode)this.getParent()).getAllOntologies(_ontologies);
...
//	" right_type=" + _right_type + " _user_id=" + _user_id + " _child_node_type=" + _child_node_type);
		boolean result = false;
		if(this.getNodeType() == iNode.NODE_TYPE_I_COPE_SEARCH)
			return result;
...
//if(this.xtrnl_object != null) System.out.println("	Node.isCreatedBy external creator is null="+xtrnl_object.getCreator());
		if( ((this.creator != null) && (this.xtrnl_object == null) &&
				(this.creator.getId() == _user_id) ) || ((this.xtrnl_object != null)&&
			(this.xtrnl_object.isCreatedBy(_user_id))) )
			return true;
		else
			return false;
	}
...
//if(this.xtrnl_object != null) System.out.println("	Node.isCreatedBy external creator is null="+xtrnl_object.getCreator());
		if( ((this.creator != null) && (this.xtrnl_object == null)) &&
			(this.creator.getId() == _user.getId()) || ((this.xtrnl_object != null)&&
			(this.xtrnl_object.isCreatedBy(_user))) )
			return true;
		else
			return false;
	}
	...
	* @param _user - the creator of the external object */
	...
	// Children property
	public OrderedWeightedItemVector<iNode> getChildren() { return children; }
...
//if((AppDaemon.CONTEXT_ASSETS_PATH_VAL==null))System.out.println("(AppDaemon.CONTEXT_ASSETS_PATH_VAL==null):  " + (AppDaemon.CONTEXT_ASSETS_PATH_VAL==null));
		...
		// folder ident image
		if(level > 0)
			out.print("<img src='" + request.getContextPath() + ((ClientDaemon.CONTEXT_ASSETS_PATH!=null)?ClientDaemon.CONTEXT_ASSETS_PATH:"/assets") + "/dir_empty.gif' height='16' width='"+ (level * 18) + "'alt='.    ' border=0 " + "style='vertical-align:baseline;display:inline'/>");
...
//System.out.println("(content_greedy_n_trace && !show_nolink) = " + (content_greedy_n_trace && !show_nolink) + " show_nolink="+show_nolink);
				...
				{
					out.print("<img src='" + request.getContextPath() + ((ClientDaemon.CONTEXT_ASSETS_PATH!=null)?ClientDaemon.CONTEXT_ASSETS_PATH:"/assets") + "/" + ((getExpanded())?"dir_minus.gif":"dir_plus.gif") + "' alt='[-]' border=0 " + "style='vertical-align:baseline;display:inline'>");
				}
				else // pack both icons
				{
					out.print("<img id='n" + getId() + "minus' src='" + request.getContextPath() + ((ClientDaemon.CONTEXT_ASSETS_PATH!=null)?ClientDaemon.CONTEXT_ASSETS_PATH:"/assets") + "/dir_minus.gif' alt='[-]' border=0 " + "style='vertical-align:baseline;display:" + ((getExpanded())?"inline":"none") +"'>");
					out.print("<img id='n" + getId() + "plus' src='" + request.getContextPath() + ((ClientDaemon.CONTEXT_ASSETS_PATH!=null)?ClientDaemon.CONTEXT_ASSETS_PATH:"/assets") + "/dir_plus.gif'  alt='[+]' border=0 " + "style='vertical-align:baseline;display:" + ((getExpanded())?"none":"inline") +"'>");
				}
				out.print("</a>");
			}
			else // no children
			{
				out.print("<a name='n" + getId() + "a' border=0 style='display:inline'><img src='" + request.getContextPath() + ((ClientDaemon.CONTEXT_ASSETS_PATH!=null)?ClientDaemon.CONTEXT_ASSETS_PATH:"/assets") + "/dir_empbull.gif'" + " alt='&curren;' border=0 style='vertical-align:baseline;display:inline'/></a>");
			}
		}
		...
		// print an icon
		out.print("&nbsp;<img src='" + request.getContextPath() + ((ClientDaemon.CONTEXT_ASSETS_PATH!=null)?ClientDaemon.CONTEXT_ASSETS_PATH:"/assets") + "/" + icon_file + "' alt='' border=0 " + "style='vertical-align:baseline;display:inline'>");
...
//System.out.println("Node.outputTree Node.id=" + this.getId() + "can_see_author=" + can_see_author);
		...
		out.print("&nbsp;<a" + ((user!=null && isCreatedBy(user.getId())) ? " style='text-decoration:underline;color:#006600;'" : "") + " class='" + style + "' title='" + display_title + "'" + href + ">" + display_title + "</a>");
...
//		if(node.user_id==user_id) style = "pt_menu_left_link_owned";
	...
	public ItemVector<iConcept> getOntologies() {return ontologies;}

	public float getProfileMatch(User _user)
	{
//System.out.println("======= " + this.getTitle()); /// DEBUG
		...
		for(int i=0; i<this.children.size(); i++)
			if(this.children.get(i).getNodeType() ==
				iNode.NODE_TYPE_I_CONCEPTS)
			{
				nc = (NodeConcepts)this.children.get(i).
					getExternalObject();
				break;
			}
		if(nc != null)
		{
			nc_count = nc.getConcepts().size();
			for(int i=0; i<nc.getConcepts().size(); i++)
			{
//System.out.println("--" + i + "/" + nc.getConcepts().size() + "-#Node.getProfileMatch examining='" + nc.getConcepts().get(i).getTitle()); /// DEBUG
//System.out.println("#Node.getProfileMatch nc.getConcepts().get(i)=null? " + (nc.getConcepts().get(i)==null)); /// DEBUG
//System.out.println("#Node.getProfileMatch found=null? " + (_user.getConcepts().findById(nc.getConcepts().get(i).getId())==null)); /// DEBUG
				if(_user.getConcepts().findById(nc.getConcepts().
					get(i).getId()) != null)
				{
					nc_overlap ++;
...
//System.out.println(""); /// DEBUG
		...
		if(nc_count != 0)
		{
//System.out.println("Node.getProfileMatch title='" + this.getTitle() + "' match=" + ((float)nc_overlap/nc_count)); /// DEBUG
			result = (float)nc_overlap/nc_count;
		}
		return result;
	}
...
//System.out.println("Node.showAdd starting...");
		...
		out.println("<input name='fld_id' type='hidden' value='" + this.getId() + "'>");
		...
		switch (this.node_type)
		{
			...
			case iNode.NODE_TYPE_I_PAPER:
				node_type_name = "Paper";
			break;
			case iNode.NODE_TYPE_I_SUMMARY:
				node_type_name = "Summary";
			break;
			case iNode.NODE_TYPE_I_CONCEPTS:
				node_type_name = "Concepts";
			break;
		}
...
//		long node_types = 0;
//System.out.println("NodeUntyped.showAdd rights # " + getRights().size());
...
//			}
//		}
//System.out.println("NodeUntyped.showAdd all_types = " + all_types);
...
//			res_map.isAllowedWhatWhoForFromTo(Right.RIGHT_TYPE_ADD, group_id, this, this.getNodeType(), NODE_TYPES_I_DEFINITIVE[i]))
			...
			if(res_map.isAllowedWhatWho2ForFromToQuant(Right.RIGHT_TYPE_ADD,
				user_id, group_id, this, this.getNodeType(),NODE_TYPES_I_DEFINITIVE[i]))
			{
...
//System.out.println("Node.findChildOfTypeByUser _node_type = " + _node_type);
	...
	public boolean canSeeAuthor(User user, User group, ResourceMap resmap)
	{
		...
		if((this.getNodeType() != iNode.NODE_TYPE_I_MYPROFILE) &&
			(this.getNodeType() != iNode.NODE_TYPE_I_COPE_SEARCH) &&
			(resmap != null))
		{
			can_see_author = (
				resmap.isAllowedWhatWhoForFromTo(
...
//		User user = this.getCreator();
		...
		String url = (this.getURL()!=null) ? 
			"'" + this.getURL() + "'" : "NULL";
		String parameters = (((NodeUntyped)this).getProperties()!=null) ? 
			"'" + ((NodeUntyped)this).getProperties() + "'" : "NULL";
		String qry = "INSERT INTO ent_node (Title, UserID, DateCreated, " +
			...
			ext_s + ", " + description + ", " + parameters + "," + url + ");";
		...
		try
		{
			...
			new_node = new NodeUntyped(new_node_id, this.getTitle(), 
				this.getNodeType(), this.getDescription(), this.getURL(), 
				this.getFolderFlag(), ((NodeUntyped)this).getProperties(),
				user, this.getExternalObject());
