...
public class Right
{
	...
	protected int node_macro; // node macro code
	...
	public int getNodeMacro() { return node_macro; }
...
//System.out.println("Right.isAllowedWho result " + result);
	...
	{
		if( (this.node_macro == Right.NODE_ALL) || 
			(this.node.getId() == _node_id) )
			return true;
		else
			return false;
	}
...
//System.out.println("\t\t isAllowedTo(_child_node_type) " + isAllowedTo(_child_node_type));
		if( isAllowedWhatWhoForFrom(_right_type, _user_id, 
			...
			!((_right_type == Right.RIGHT_TYPE_DELETE || 
					_right_type == Right.RIGHT_TYPE_COPY ||
					_right_type == Right.RIGHT_TYPE_RATE ) 
